<template>
  <div class="draw">
    <div>
      <div>
        <div :class="resData.IdBox" class="">
            <div ref='box' :class="resData.Flex" :style="!drawXY ? 'display: flex;':''">
             <div v-for="(item, index) in drawDataArr" :key="index">
               <div :style="`width:${resData.WH.width}px;height:${resData.WH.height}px`"
                   ...
                   <img width="100px" height="" :class="resData.classImg" :src="item.src" :alt="item.name">
                   <div>{{item.name}}</div>
                   <slot></slot>
               </div>
            </div>
        ...
        <div class="green" @click="playClick"></div>
        <div class="green" @click="playClickStop">playClick - {{drawSuccess}}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    // 列表数据 - 至少传输五个
    drawData: {type:Array,default(){return []}},
    ...
    // 中奖的数据
    drawDataSuccess: {type:Object,default(){
    ...
    // 执行动画的函数
    getStart:{type:Function,default(){return ()=>{console.log('动画开始')}}},
    getUpdate:{type:Function,default(){return ()=>{console.log('进行中')}}},
    getComplete:{type:Function,default(){return ()=>{console.log('已结束')}}},
    getRepeat:{type:Function,default(){return ()=>{console.log('......')}}},
  },
  data() {
    return {
      play: null,
  ...
  computed:{
    ...
    drawSuccess(){
      ...
      console.log('drawSuccess Y周滚动  高度 否则是宽度')
      console.log(index)
    ...
    drawDataArr(){
      ...
      console.log(this.drawSuccess + this.winIndex-1)
